# 이 부분 정말 짱.
# i 가 다 실행되지 않아서 돌아가는거 이해하기
# 내가 실행했던 i 는 다른 함수의 i 일수도 있는 점 유의하기

"""
backtrack(1, [])

i=1 선택 → backtrack(2, [1])

            i=2 선택 → backtrack(3, [1, 2]) → 저장!

            i=3 선택 → backtrack(4, [1, 3]) → 저장!

i=2 선택 → backtrack(3, [2])

            i=3 선택 → backtrack(4, [2, 3]) → 저장!

i=3 선택 → backtrack(4, [3])

for문 범위(4~3) 미달로 즉시 종료
"""
